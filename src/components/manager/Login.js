...
function Login(){
    ...
    const login = (e) => {
        e.preventDefault();
        axios.post(`${APILink}/manager/login`,{username:username, password:password})
            .then(res => {
                if(res.data.status === "SUCCESS"){
                    localStorage.setItem("manager-token",res.data.id);
    ...
    if(localStorage.getItem("manager-token") !==null){
        return <Redirect to={Routes.managerDashboard.path} />
    }
    else{
        return(
            <>
                <Title title={`${Name} | ${Routes.managerLogin.name}`} />
                <GridContainer>
                    <GridItem xs={12} sm={12} md={12}>
                        <div style={loginBox}>
                            <h2 className="text-center">Manager Panel</h2>
                            <hr/><br/>
